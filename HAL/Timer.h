...
#define CLOCK_CYCLES_IN_MS     (SYSTEM_CLOCK/1000) //number of clock cycles in 1ms
...
// Determines if the timer has expired - i.e. if enough time has passed since the timer was started
...
void startHWTimer(uint64_t waitTime_ms);
bool HWTimerExpired();
